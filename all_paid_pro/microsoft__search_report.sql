{% assign dimensions = vars.microsoft_advertising.models.microsoft__search_report.dimensions %}
...
{% assign metrics = vars.microsoft_advertising.models.microsoft__search_report.metrics %}
...
{% assign table_active = vars.microsoft_advertising.models.microsoft__search_report.active %}
{% assign dataset_id = vars.output_dataset_id %}
{% assign table_id = vars.microsoft_advertising.models.microsoft__search_report.table_id %}
...
{% assign source_table_id = 'search_query_performance_daily_report' %}
...
{% if active and table_active %}
    AS(
        ...
        , campaigns as(
            ...
            qualify rank() over(partition by id order by modified_time desc) = 1
            and row_number() over(partition by id, modified_time) = 1
        )
        ...
        , report as(
            select
                ...
                , delivered_match_type as search_term_match_type
                , sum(spend) as cost
            ...
            from
                (
                    select
                        ...
                        , goal
                    from
                        {{source_dataset_id}}.goals_and_funnels_daily_report
                )
            pivot(
                /* 
        ...
        , api as(
            select
                ...
                , keywords.name as keyword_text
                , keywords.match_type as keyword_match_type
                , report.search_term
                , report.search_term_match_type
                , report.clicks
                , report.cost
                , report.impressions
                , report.conversions
